...
class CAPSCALE(object):
    def __init__(self, file_path, input_files, output_file_name, ref_value, **kwargs):
        ...
        self.cap_inject_5 = False  # alternative for including injection options, 5 pF
        self.cap_inject_10 = False  # alternative for including injection options, 10 pF
            ...
            for row in reader:
                ...
                if row[0] == 'Date':
                    ...
                    if len(row)==3: # configuration of 0.5 pF bridge
                        if row[2]=='cap inject':
                            self.cap_inject = True  # injection through 5 pF capacitor
                        elif row[2]=='cap inject 5':
                            self.cap_inject_5 = True  # injection through 5 pF capacitor
                        elif row[2]=='cap inject 10':
                            self.cap_inject_10 = True  # injection through 5 pF capacitor
    ...
    def sum_ratio3(self, bal1, bal2, sum_val, cap1, cap2, com_lead):
        ...
        A = (1 + rbal2 * admit_ES13 / admit_ES16) / (1 + rbal1 * admit_ES16 / admit_ES13) # eqn 10 'ES14 modification'
        c13 = sum_val * (A / (1 + A))
        c16 = sum_val / (1 + A)
        c14 = c16 / self.main_ratio * (1 + rbal2 * admit_ES13 / admit_ES16)
        return c13, c16, c14

    def sum_ratio4(self, bal1, bal2, sum_val, cap1, cap2, com_lead):
        """
        Calculates the value of the two 5 pF capacitors (ES13 and ES16) from knowing the value of the two in parallel
        and their difference in ratio when individually balanced against the 0.5 pF (ES14). The 5 pF capacitors connect
        directly to the HV side when measured against a 100 pF capacitor, but are on the LV side when being compared to
        the 0.5 pF capacitor. The bal1 and bal2 measurements are made with GR10 as the injection capacitor
        connected to the 10:1 transformer (similar to the permutable bridge).
        E005.003 does not have these formulae.
        ...
        admit_ES13 = cap1.best_value  # admittance at 10000 rad/s
        admit_ES16 = cap2.best_value
        admit_gr10 = self.caps['gr10'].best_value  # this is the injection capacitor for the sum_ratio4 option
        rbal1 = r * (bal1[0] * self.factora + 1j * bal1[1] * self.factorb)
        rbal2 = r * (bal2[0] * self.factora + 1j * bal2[1] * self.factorb)
        A = (1 + rbal2 * admit_gr10 / admit_ES16) / (1 + rbal1 * admit_gr10 / admit_ES13)
        c13 = sum_val * (A / (1 + A))
        c16 = sum_val / (1 + A)
        c14 = c16 / self.main_ratio * (1 + rbal2 * admit_gr10 / admit_ES16)  # equation 9 'ES14 modification'
        return c13, c16, c14
    ...
    def buildup(self):
        ...
        if self.cap_inject:  # old csv default to 5 pF injection
            print('cap_inject')
            c13_2, c16_2, c14_2 = self.sum_ratio3(r1, r2, e1316, self.caps['es13'], self.caps['es16'], self.leads['xfrm'])
        elif self.cap_inject_5:  # specific choice of 5 pF injection
            print('cap_inject_5')
            c13_2, c16_2, c14_2 = self.sum_ratio3(r1, r2, e1316, self.caps['es13'], self.caps['es16'],
                                                  self.leads['xfrm'])
        elif self.cap_inject_10:  # specific choice of 10 pF injection
            print('cap_inject_10')
            c13_2, c16_2, c14_2 = self.sum_ratio4(r1, r2, e1316, self.caps['es13'], self.caps['es16'],
                                                  self.leads['xfrm'])
        else:
            print('volt_inject')
